...
import { capSavedFilters, parseSavedFilters, SAVED_FILTERS_KEY, serializeSavedFilters, type SavedFilter } from "@/lib/saved-filters";
...
export function UnifiedFilters({ visible, language, regionScope, category, subcategory, sort, kitchenStatuses, calorieRanges, onRegionChange, onCategoryChange, onSubcategoryChange, onSortChange, onKitchenStatusesChange, onCalorieRangesChange, onClose }: UnifiedFiltersProps) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(SAVED_FILTERS_KEY).then((value) => {
      if (!value) return;
      setSavedFilters(parseSavedFilters(value));
    }).catch(() => undefined);
  }, []);
  ...
  const persistSavedFilters = (next: SavedFilter[]) => { const capped = capSavedFilters(next); setSavedFilters(capped); void AsyncStorage.setItem(SAVED_FILTERS_KEY, serializeSavedFilters(capped)); };
  const saveCurrentFilters = () => {
    const name = saveName.trim();
    if (!name) return;
    const saved: SavedFilter = { id: `${Date.now()}`, name, regionScope, category, subcategory, sort, kitchenStatuses, calorieRanges };
    persistSavedFilters([saved, ...savedFilters].slice(0, 10));
    setSaveName("");
  };
  const applySavedFilter = (saved: SavedFilter) => { onRegionChange(saved.regionScope); onCategoryChange(saved.category); onSubcategoryChange(saved.subcategory); onSortChange(saved.sort); onKitchenStatusesChange(saved.kitchenStatuses); onCalorieRangesChange(saved.calorieRanges); };
  const clearAll = () => { onRegionChange("all"); onCategoryChange("all"); onSubcategoryChange("all"); onSortChange("recommended"); onKitchenStatusesChange([]); onCalorieRangesChange([]); };
  return (
    <View style={styles.panel}>
      ...
      <View style={styles.savedHeader}><Text style={styles.sectionTitle}>{isArabic ? "الفلاتر المفضلة" : "Saved filters"}</Text><Text style={styles.savedCount}>{savedFilters.length}/10</Text></View>
      <View style={styles.saveRow}><TextInput value={saveName} onChangeText={setSaveName} placeholder={isArabic ? "اسم مثل: أكل خفيف" : "Name, e.g. Light meals"} placeholderTextColor="#8ABAC0" style={styles.saveInput} textAlign={isArabic ? "right" : "left"} returnKeyType="done" onSubmitEditing={saveCurrentFilters} /><Pressable disabled={!saveName.trim()} onPress={saveCurrentFilters} style={({ pressed }) => [styles.saveButton, !saveName.trim() && styles.saveButtonDisabled, pressed && styles.pressed]}><MaterialIcons name="bookmark-add" size={16} color="#FFFFFF" /><Text style={styles.saveButtonText}>{isArabic ? "حفظ الحالي" : "Save current"}</Text></Pressable></View>
      {savedFilters.length > 0 && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.savedRow}>{savedFilters.map((saved) => <View key={saved.id} style={styles.savedChip}><Pressable onPress={() => applySavedFilter(saved)} style={styles.savedApply}><MaterialIcons name="bookmark" size={14} color="#00AFC4" /><Text style={styles.savedName} numberOfLines={1}>{saved.name}</Text></Pressable><Pressable onPress={() => persistSavedFilters(savedFilters.filter((item) => item.id !== saved.id))} style={styles.savedDelete}><MaterialIcons name="close" size={13} color="#C4555D" /></Pressable></View>)}</ScrollView>}
...
const styles = StyleSheet.create({
  ...
  savedHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 2 }, savedCount: { color: "#8ABAC0", fontSize: 10, fontWeight: "800" }, saveRow: { flexDirection: "row", alignItems: "center", gap: 7 }, saveInput: { flex: 1, minHeight: 38, borderWidth: 1, borderColor: "#D2EFF2", borderRadius: 12, paddingHorizontal: 10, paddingVertical: 7, color: "#082E34", fontSize: 11, backgroundColor: "#F7FCFD" }, saveButton: { minHeight: 38, flexDirection: "row", alignItems: "center", gap: 5, borderRadius: 12, paddingHorizontal: 10, backgroundColor: "#2E9B72" }, saveButtonDisabled: { opacity: 0.45 }, saveButtonText: { color: "#FFFFFF", fontSize: 10, fontWeight: "900" }, savedRow: { gap: 7, paddingVertical: 1 }, savedChip: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: "#BCEFF4", borderRadius: 12, backgroundColor: "#F0FCFD", overflow: "hidden" }, savedApply: { flexDirection: "row", alignItems: "center", gap: 5, paddingVertical: 8, paddingLeft: 9, paddingRight: 5 }, savedName: { maxWidth: 130, color: "#087B8A", fontSize: 10, fontWeight: "900" }, savedDelete: { paddingHorizontal: 7, paddingVertical: 8, borderLeftWidth: 1, borderLeftColor: "#BCEFF4" },
  sectionTitle: { color: "#082E34", fontSize: 11, fontWeight: "900", marginTop: 4 }, chipRow: { gap: 7, paddingVertical: 1 }, chip: { borderWidth: 1, borderColor: "#D2EFF2", backgroundColor: "#F7FCFD", borderRadius: 12, paddingHorizontal: 10, paddingVertical: 8 }, chipActive: { backgroundColor: "#00AFC4", borderColor: "#00AFC4" }, chipText: { color: "#4C747A", fontSize: 10, fontWeight: "800" }, chipTextActive: { color: "#FFFFFF" }, optionGrid: { flexDirection: "row", flexWrap: "wrap", gap: 7 }, statusOption: { flexDirection: "row", alignItems: "center", gap: 5, borderWidth: 1, borderColor: "#D2EFF2", backgroundColor: "#F7FCFD", borderRadius: 12, paddingHorizontal: 9, paddingVertical: 8 }, statusOptionActive: { backgroundColor: "#2E9B72", borderColor: "#2E9B72" }, calorieOption: { flexDirection: "row", alignItems: "center", gap: 5, borderWidth: 1, borderColor: "#F0D6A0", backgroundColor: "#FFF9ED", borderRadius: 12, paddingHorizontal: 9, paddingVertical: 8 }, calorieOptionActive: { backgroundColor: "#C98A2E", borderColor: "#C98A2E" }, optionText: { color: "#4C747A", fontSize: 10, fontWeight: "800" }, optionTextActive: { color: "#FFFFFF" }, sortGrid: { flexDirection: "row", flexWrap: "wrap", gap: 7 }, sortOption: { flexDirection: "row", alignItems: "center", gap: 5, borderWidth: 1, borderColor: "#D2EFF2", backgroundColor: "#F7FCFD", borderRadius: 12, paddingHorizontal: 9, paddingVertical: 8 }, sortOptionActive: { backgroundColor: "#00AFC4", borderColor: "#00AFC4" }, sortOptionText: { color: "#4C747A", fontSize: 10, fontWeight: "800" }, sortOptionTextActive: { color: "#FFFFFF" }, clearAllButton: { width: "100%", minHeight: 42, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, borderRadius: 13, backgroundColor: "#00AFC4", borderWidth: 1, borderColor: "#00AFC4", marginTop: 4 }, clearAllButtonIdle: { backgroundColor: "#F0FCFD", borderColor: "#BCEFF4" }, clearAllText: { color: "#FFFFFF", fontSize: 11, fontWeight: "900" }, clearAllTextIdle: { color: "#00AFC4" }, clearPressed: { opacity: 0.82, transform: [{ scale: 0.98 }] }, pressed: { opacity: 0.82, transform: [{ scale: 0.98 }] },
});
