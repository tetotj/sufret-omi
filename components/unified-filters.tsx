...
export function UnifiedFilters({ visible, language, regionScope, category, subcategory, sort, kitchenStatuses, calorieRanges, onRegionChange, onCategoryChange, onSubcategoryChange, onSortChange, onKitchenStatusesChange, onCalorieRangesChange, onClose }: UnifiedFiltersProps) {
  ...
  return (
    <View style={styles.panel}>
      ...
      <Pressable onPress={() => { onRegionChange("all"); onCategoryChange("all"); onSubcategoryChange("all"); onSortChange("recommended"); onKitchenStatusesChange([]); onCalorieRangesChange([]); }} style={styles.clearButton}><MaterialIcons name="restart-alt" size={16} color="#00AFC4" /><Text style={styles.clearText}>{isArabic ? "مسح الفلاتر" : "Clear filters"}</Text></Pressable>
    </View>
  );
}
