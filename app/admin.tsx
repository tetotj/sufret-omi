...
function AdminAnalytics({ language, useDatabase }: { language: "ar" | "en"; useDatabase: boolean }) {
  ...
  const scheduleMutation = trpc.admin.ensureWeeklyReportSchedule.useMutation();
  const [scheduleState, setScheduleState] = useState<"idle" | "success" | "error">("idle");
  const data = analyticsQuery.data;
  const weekly = weeklyQuery.data;
  const enableWeeklyEmail = () => {
    setScheduleState("idle");
    scheduleMutation.mutate(undefined, { onSuccess: () => setScheduleState("success"), onError: () => setScheduleState("error") });
  };
...
function AdminUsers({ language, showToast, useDatabase }: { language: "ar" | "en"; showToast: (message: string) => void; useDatabase: boolean }) {
  ...
  const update = (user: ManagedUser, status: UserAccountStatus) => {
    if (useDatabase) {
      updateMutation.mutate({ userId: user.id, status }, { onSuccess: () => { if (user.role !== "customer") setVerificationApproval(user.role, status === "active" ? "approved" : "rejected"); remoteQuery.refetch(); setSelected(null); showToast(language === "ar" ? `تم تغيير حالة ${user.name} في قاعدة البيانات` : `${user.name}'s database status changed`); }, onError: () => showToast(language === "ar" ? "تعذر تحديث المستخدم من الخادم" : "Could not update user on the server") });
      return;
    }
    updateUserStatus(user.id, status); if (user.role !== "customer") setVerificationApproval(user.role, status === "active" ? "approved" : "rejected"); setSelected(null); showToast(language === "ar" ? `تم تغيير حالة ${user.name}` : `${user.name}'s status changed`);
  };
...
function AdminApprovals({ language, showToast, useDatabase }: { language: "ar" | "en"; showToast: (message: string) => void; useDatabase: boolean }) {
  ...
  const remoteQuery = trpc.admin.listUsers.useQuery(undefined, { enabled: useDatabase });
  const updateMutation = trpc.admin.updateUserStatus.useMutation();
  const descriptionQuery = trpc.admin.listPendingKitchenDescriptions.useQuery(undefined, { enabled: useDatabase });
  const descriptionDecisionMutation = trpc.admin.decideKitchenDescription.useMutation();
  const mealQuery = trpc.admin.listPendingMealApprovals.useQuery(undefined, { enabled: useDatabase });
  const mealDecisionMutation = trpc.admin.decideMealApproval.useMutation();
  const managedUsers = useDatabase ? ((remoteQuery.data ?? []) as unknown as ManagedUser[]) : localManagedUsers;
...
function KitchenDescriptionApprovals({ language, items, onDecide }: { language: "ar" | "en"; items: PendingKitchenDescriptionView[]; onDecide: (kitchenId: string, status: "approved" | "rejected") => void }) {
  return <View style={styles.panel}><View style={styles.panelHeader}><View><Text style={styles.panelTitle}>{language === "ar" ? "أوصاف بانتظار الموافقة" : "Descriptions awaiting approval"}</Text><Text style={styles.panelSubtitle}>{language === "ar" ? "لا يظهر الوصف للعميل قبل اعتماد المشرف." : "Descriptions stay hidden until the supervisor approves them."}</Text></View><View style={styles.pendingCounter}><Text style={styles.pendingCounterValue}>{items.length}</Text><Text style={styles.pendingCounterLabel}>{language === "ar" ? "وصف" : "items"}</Text></View></View>{items.length === 0 ? <EmptyPanel icon="description" text={language === "ar" ? "لا توجد أوصاف جديدة" : "No new descriptions"} /> : <View style={styles.approvalList}>{items.map((item) => <View key={item.kitchenId} style={styles.approvalCard}><Text style={styles.userName}>{language === "ar" ? item.kitchenNameAr : item.kitchenNameEn}</Text><Text style={styles.userMeta}>{language === "ar" ? `بواسطة ${item.motherNameAr}` : `Submitted by ${item.motherNameEn}`}</Text><View style={styles.detailBox}><Text style={styles.detailBoxText}>{language === "ar" ? item.descriptionAr || "لا يوجد وصف عربي" : item.descriptionEn || "No English description"}</Text></View><View style={styles.detailActions}><Pressable onPress={() => onDecide(item.kitchenId, "rejected")} style={styles.rejectButton}><Text style={styles.rejectButtonText}>{language === "ar" ? "رفض" : "Reject"}</Text></Pressable><Pressable onPress={() => onDecide(item.kitchenId, "approved")} style={styles.approveButton}><MaterialIcons name="verified" size={16} color="#FFFFFF" /><Text style={styles.approveButtonText}>{language === "ar" ? "اعتماد ونشر" : "Approve & publish"}</Text></Pressable></View></View>)}</View>}</View>;
}

type PendingMealApprovalView = {
  mealId: string;
  kitchenId: string;
  nameAr: string;
  nameEn: string;
...
function MealApprovals({ language, items, onDecide }: { language: "ar" | "en"; items: PendingMealApprovalView[]; onDecide: (mealId: string, status: "approved" | "rejected") => void }) {
  return <View style={styles.panel}><View style={styles.panelHeader}><View><Text style={styles.panelTitle}>{language === "ar" ? "طبخات بانتظار النشر" : "Meals awaiting publication"}</Text><Text style={styles.panelSubtitle}>{language === "ar" ? "لا تظهر الطبخة للعميل قبل اعتمادها." : "Meals stay unavailable until approved."}</Text></View><View style={styles.pendingCounter}><Text style={styles.pendingCounterValue}>{items.length}</Text><Text style={styles.pendingCounterLabel}>{language === "ar" ? "طبخة" : "meals"}</Text></View></View>{items.length === 0 ? <EmptyPanel icon="restaurant-menu" text={language === "ar" ? "لا توجد طبخات بانتظار النشر" : "No meals awaiting publication"} /> : <View style={styles.approvalList}>{items.map((item) => <View key={item.mealId} style={styles.approvalCard}><Text style={styles.userName}>{language === "ar" ? item.nameAr : item.nameEn}</Text><Text style={styles.userMeta}>{item.kitchenId} · {item.category} · {item.price} JOD</Text><View style={styles.detailActions}><Pressable onPress={() => onDecide(item.mealId, "rejected")} style={styles.rejectButton}><Text style={styles.rejectButtonText}>{language === "ar" ? "رفض" : "Reject"}</Text></Pressable><Pressable onPress={() => onDecide(item.mealId, "approved")} style={styles.approveButton}><MaterialIcons name="verified" size={16} color="#FFFFFF" /><Text style={styles.approveButtonText}>{language === "ar" ? "اعتماد ونشر" : "Approve & publish"}</Text></Pressable></View></View>)}</View>}</View>;
}
