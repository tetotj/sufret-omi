...
import { complaintCategories, complaintStatuses, type ComplaintStatus } from "@/lib/complaint-data";
...
export default function AdminPage() {
  ...
  if (!adminAuthenticated) return <AdminLogin language={language} onSignIn={adminSignIn} />;
  return <AdminShell language={language} onSignOut={adminSignOut} showToast={showToast} />;
}
...
function AdminUsers({ language, showToast }: { language: "ar" | "en"; showToast: (message: string) => void }) {
  const { managedUsers, updateUserStatus } = useApp();
  ...
  const update = (user: ManagedUser, status: UserAccountStatus) => { updateUserStatus(user.id, status); setSelected(null); showToast(language === "ar" ? `تم تغيير حالة ${user.name}` : `${user.name}'s status changed`); };
...
function AdminApprovals({ language, showToast }: { language: "ar" | "en"; showToast: (message: string) => void }) {
  const { managedUsers, updateUserStatus } = useApp();
  const pending = managedUsers.filter((user) => user.status === "pending_approval");
  const decide = (user: ManagedUser, status: UserAccountStatus) => { updateUserStatus(user.id, status); showToast(language === "ar" ? `تم تحديث طلب ${user.name}` : `${user.name}'s request updated`); };
...
function AdminComplaints({ language }: { language: "ar" | "en" }) {
  const { complaints, updateComplaintStatus } = useApp();
  const [responses, setResponses] = useState<Record<string, string>>({});
  const changeStatus = (id: string, status: ComplaintStatus) => updateComplaintStatus(id, status, responses[id] ?? "");
