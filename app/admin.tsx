...
import { getLocalized, meals } from "@/lib/food-data";
type AdminSection = "overview" | "users" | "approvals" | "complaints" | "marketing" | "settings";
...
type AnnouncementDraft = { id: string; eyebrowAr: string; eyebrowEn: string; titleAr: string; titleEn: string; bodyAr: string; bodyEn: string; ctaAr: string; ctaEn: string; icon: string; target: "meals" | "orders"; sortOrder: string; isActive: boolean };
type OfferDraft = { id: string; mealId: string; badgeAr: string; badgeEn: string; discountPercent: string; sortOrder: string; isActive: boolean };
const emptyAnnouncementDraft: AnnouncementDraft = { id: "", eyebrowAr: "", eyebrowEn: "", titleAr: "", titleEn: "", bodyAr: "", bodyEn: "", ctaAr: "", ctaEn: "", icon: "campaign", target: "meals", sortOrder: "1", isActive: true };
const emptyOfferDraft: OfferDraft = { id: "", mealId: meals[0]?.id ?? "", badgeAr: "", badgeEn: "", discountPercent: "", sortOrder: "1", isActive: true };
...
function AdminMarketing({ language, showToast, useDatabase }: { language: "ar" | "en"; showToast: (message: string) => void; useDatabase: boolean }) {
  ...
  const resetAnnouncement = () => { setAnnouncementDraft(emptyAnnouncementDraft); setEditingAnnouncement(null); };
  const resetOffer = () => { setOfferDraft(emptyOfferDraft); setEditingOffer(null); };
  const saveAnnouncement = async () => {
    ...
    const payload = { ...announcementDraft, sortOrder: Number(announcementDraft.sortOrder) || 0, startsAt: null, endsAt: null };
    try {
  ...
  const saveOffer = async () => {
    ...
    const payload = { ...offerDraft, discountPercent: offerDraft.discountPercent ? Number(offerDraft.discountPercent) : null, sortOrder: Number(offerDraft.sortOrder) || 0, startsAt: null, endsAt: null };
    try {
  ...
  const editAnnouncement = (item: AnnouncementRecord) => { setEditingAnnouncement(item.id); setAnnouncementDraft({ ...item, sortOrder: String(item.sortOrder), isActive: item.isActive }); };
  const editOffer = (item: OfferRecord) => { setEditingOffer(item.id); setOfferDraft({ ...item, discountPercent: item.discountPercent ?? "", sortOrder: String(item.sortOrder), isActive: item.isActive }); };
