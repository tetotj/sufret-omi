...
<MaterialIcons name={driverOrder.status === "ready" ? "shopping-bag" : "check-circle"} size={19} color="#FFFFFF" /><Text style={styles.driverActionButtonText}>{actionLabel}</Text></Pressable> : <View style={styles.driverDone}><MaterialIcons name="check-circle" size={21} color="#2E9B72" /><Text style={styles.driverDoneText}>{language === "ar" ? "تمت التوصيلة بنجاح، يعطيك العافية" : "Delivery complete, great work"}</Text></View>}
      </> : <View style={styles.driverDone}><MaterialIcons name="local-cafe" size={21} color="#D76545" /><Text style={styles.driverDoneText}>{language === "ar" ? "ما في طلبات قريبة حالياً" : "No nearby orders right now"}</Text></View>}
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.unifiedControlsRow}>
        <LanguageToggle />
        <View style={styles.unifiedSearchField}><MaterialIcons name="search" size={18} color="#7E665A" /><TextInput value={query} onChangeText={setQuery} placeholder={language === "ar" ? "ابحثي" : "Search"} placeholderTextColor="#CDB4A5" style={styles.unifiedSearchInput} textAlign={language === "ar" ? "right" : "left"} /></View>
        <Pressable onPress={() => setFiltersOpen((value) => !value)} style={({ pressed }) => [styles.unifiedIconButton, filtersOpen && styles.unifiedIconButtonActive, pressed && styles.pressed]}><MaterialIcons name="tune" size={18} color={filtersOpen ? "#FFFFFF" : "#D76545"} /></Pressable>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryRow}>
        <CategoryPill label={language === "ar" ? "الكل" : "All"} icon="apps" color="#D76545" selected={selectedCategory === "all"} onPress={() => { setSelectedCategory("all"); onNavigate("meals"); }} />
      ...
      {activeOrder && (
        <Pressable onPress={() => onNavigate("orders")} style={styles.activeOrderCard}>
          ...
          <View style={styles.activeOrderBody}><View><Text style={styles.activeOrderTitle}>{getLocalized(activeOrder.kitchen.name, language)}</Text><Text style={styles.activeOrderMeta}>{getLocalized(activeOrder.eta, language)}</Text></View><MaterialIcons name="chevron-right" size={22} color="#D76545" /></View>
        </Pressable>
      )}
...
function BottomNav({ active, onNavigate, role, language }: { active: ViewId; onNavigate: (view: ViewId) => void; role: Role; language: "ar" | "en" }) {
  ...
  return <View style={styles.bottomNav}>{items.map((item) => <Pressable key={item.id} onPress={() => onNavigate(item.id)} style={({ pressed }) => [styles.navItem, pressed && styles.pressed]}><MaterialIcons name={item.icon} size={21} color={active === item.id ? "#D76545" : "#CDB4A5"} /><Text style={[styles.navLabel, active === item.id && styles.navLabelActive]}>{item.label}</Text></Pressable>)}<View style={styles.navBrandDot}><MaterialIcons name={role === "mother" ? "storefront" : "restaurant"} size={18} color="#FFFFFF" /></View></View>;
}
...
const styles = StyleSheet.create({
  ...
  loginLanguageText: { color: "#D76545", fontSize: 11, fontWeight: "900" },
  loginBrand: { alignItems: "center", gap: 2, paddingVertical: 5 },
  loginBrandArabic: { color: "#D76545", fontSize: 32, fontWeight: "900" },
  loginBrandEnglish: { color: "#38231C", fontSize: 16, fontWeight: "900", letterSpacing: 1.2 },
  loginTagline: { color: "#7E665A", fontSize: 11, marginTop: 4 },
  loginCard: { backgroundColor: "#FFFFFF", borderRadius: 24, borderWidth: 1, borderColor: "#F1D8C8", padding: 17, gap: 10, shadowColor: "#38231C", shadowOpacity: 0.06, shadowRadius: 13, shadowOffset: { width: 0, height: 5 }, elevation: 3 },
  ...
  loginTabActive: { backgroundColor: "#D76545" },
  loginTabText: { color: "#7E665A", fontSize: 11, fontWeight: "900" },
  loginTabTextActive: { color: "#FFFFFF" },
  loginTitle: { color: "#38231C", fontSize: 22, fontWeight: "900", marginTop: 5 },
  loginSubtitle: { color: "#7E665A", fontSize: 11, lineHeight: 17, marginBottom: 3 },
  inputLabel: { color: "#1A4B52", fontSize: 11, fontWeight: "900", marginTop: 2 },
  inputWrap: { height: 47, borderRadius: 15, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#FFF8F0", paddingHorizontal: 12, flexDirection: "row", alignItems: "center", gap: 8 },
  loginInput: { flex: 1, color: "#38231C", fontSize: 13, paddingVertical: 0 },
  loginError: { color: "#C4555D", fontSize: 10, fontWeight: "800", lineHeight: 15 },
  rolePrompt: { color: "#38231C", fontSize: 11, fontWeight: "900", marginTop: 4 },
  roleChoiceRow: { flexDirection: "row", gap: 8 },
  roleChoice: { flex: 1, minHeight: 54, borderRadius: 15, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center", gap: 4 },
  roleChoiceActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  ...
  guestButtonText: { color: "#D76545", fontSize: 11, fontWeight: "900" },
  ...
  logoutText: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  capacitySettingsCard: { flexDirection: "row", alignItems: "center", gap: 9, borderRadius: 17, padding: 12, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8" },
  ...
  capacitySettingsTitle: { color: "#38231C", fontSize: 11, fontWeight: "900" },
  capacitySettingsBody: { color: "#7E665A", fontSize: 10, marginTop: 3 },
  earningsBreakdown: { gap: 7, backgroundColor: "#F7FFF0", borderRadius: 14, padding: 10, borderWidth: 1, borderColor: "#F6D889" },
  driverHero: { borderRadius: 23, padding: 18, backgroundColor: "#FFF1E5", borderWidth: 1, borderColor: "#F6D889", flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  driverOverline: { color: "#C98A2E", fontSize: 10, fontWeight: "900" },
  driverTitle: { color: "#38231C", fontSize: 21, fontWeight: "900", marginTop: 5 },
  ...
  driverOrderTitle: { color: "#38231C", fontSize: 14, fontWeight: "900" },
  driverOrderMeta: { color: "#7E665A", fontSize: 10 },
  ...
  routeCard: { backgroundColor: "#FFFFFF", borderRadius: 20, padding: 14, borderWidth: 1, borderColor: "#F1D8C8" },
  ...
  routeMarkerDropoff: { backgroundColor: "#D76545" },
  routeCopy: { flex: 1 },
  routeLabel: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  routeValue: { color: "#38231C", fontSize: 12, fontWeight: "900", marginTop: 2 },
  routeCoordinates: { color: "#CDB4A5", fontSize: 10, marginTop: 3, fontVariant: ["tabular-nums"] },
  routeDistance: { color: "#D76545", fontSize: 10, fontWeight: "900", marginTop: 3 },
  ...
  driverRatingLabel: { color: "#7E665A", fontSize: 9, fontWeight: "800" },
  driverRatingValue: { color: "#38231C", fontSize: 13, fontWeight: "900", marginTop: 2 },
  ...
  driverActionDisabled: { backgroundColor: "#CDB4A5" },
  ...
  fullScreenPage: { flex: 1, backgroundColor: "#FFF8F0" },
  ...
  mealsFilterButton: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8", borderRadius: 14, paddingHorizontal: 9, paddingVertical: 7 },
  mealsFilterButtonActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  mealsFilterButtonText: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  ...
  sideFilterSheet: { width: "86%", maxWidth: 390, minHeight: "78%", maxHeight: "92%", backgroundColor: "#FFFFFF", paddingHorizontal: 18, paddingTop: 18, paddingBottom: 14, shadowColor: "#38231C", shadowOpacity: 0.2, shadowRadius: 20, elevation: 10 },
  ...
  sideFilterTitle: { color: "#38231C", fontSize: 22, fontWeight: "900" },
  sideFilterClose: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#FFF8F0", alignItems: "center", justifyContent: "center" },
  sideFilterSectionTitle: { color: "#38231C", fontSize: 13, fontWeight: "900", marginTop: 7 },
  ...
  sideFilterCheckboxActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  sideFilterCheckboxLabel: { flex: 1, color: "#193F45", fontSize: 12, fontWeight: "700" },
  sideFilterPriceRow: { flexDirection: "row", gap: 7 },
  sideFilterPriceButton: { flex: 1, minHeight: 42, borderRadius: 12, backgroundColor: "#F7FFFF", borderWidth: 1, borderColor: "#F1D8C8", alignItems: "center", justifyContent: "center" },
  sideFilterPriceButtonActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  ...
  sideFilterRadioActive: { borderColor: "#D76545" },
  sideFilterRadioDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: "#D76545" },
  ...
  sideFilterApplyButton: { flex: 1.35, minHeight: 48, borderRadius: 14, backgroundColor: "#D76545", alignItems: "center", justifyContent: "center" },
  sideFilterApplyText: { color: "#FFFFFF", fontSize: 12, fontWeight: "900" },
  fullMapArea: { flex: 1, minHeight: 360 },
  discoverSheet: { backgroundColor: "#FFF8F0", borderTopLeftRadius: 28, borderTopRightRadius: 28, borderTopWidth: 1, borderColor: "#F1D8C8", padding: 16, gap: 12 },
  ...
  discoverEyebrow: { color: "#D76545", fontSize: 10, fontWeight: "900", letterSpacing: 1 },
  discoverTitle: { color: "#38231C", fontSize: 17, fontWeight: "900", marginTop: 3 },
  discoverCount: { color: "#7E665A", fontSize: 10, fontWeight: "800", marginTop: 3 },
  nearbyPreviewRow: { flexDirection: "row", gap: 8 },
  nearbyPreview: { flex: 1, flexDirection: "row", alignItems: "center", gap: 6, padding: 9, backgroundColor: "#FFFFFF", borderRadius: 14, borderWidth: 1, borderColor: "#F1D8C8" },
  nearbyPreviewDot: { width: 8, height: 8, borderRadius: 4 },
  nearbyPreviewCopy: { flex: 1 },
  nearbyPreviewName: { color: "#38231C", fontSize: 10, fontWeight: "900" },
  nearbyPreviewDistance: { color: "#7E665A", fontSize: 9, marginTop: 2 },
  mealsIntro: { padding: 15, borderRadius: 20, backgroundColor: "#F0FBEA", borderWidth: 1, borderColor: "#C7E8C8", gap: 4 },
  mealsIntroTitle: { color: "#38231C", fontSize: 19, fontWeight: "900" },
  ...
  nearbyDistance: { color: "#D76545", fontSize: 10, fontWeight: "900", flexDirection: "row", alignItems: "center" },
  ...
  customerDashOverline: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  customerDashTitle: { color: "#38231C", fontSize: 22, fontWeight: "900", marginTop: 5 },
  ...
  dashboardTile: { width: "48%", minHeight: 92, borderRadius: 18, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8", padding: 12, gap: 8 },
  dashboardTileIcon: { width: 33, height: 33, borderRadius: 12, backgroundColor: "#F0FBEA", alignItems: "center", justifyContent: "center" },
  dashboardTileTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  dashboardTileDetail: { color: "#7E665A", fontSize: 10, marginTop: 2 },
  ...
  customerOrderTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  ...
  dashboardFootnoteText: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  ...
  eyebrow: { fontSize: 10, letterSpacing: 1.2, color: "#D76545", fontWeight: "900" },
  headerGreeting: { color: "#38231C", fontSize: 19, fontWeight: "900", marginTop: 2 },
  headerActions: { flexDirection: "row", alignItems: "center", gap: 8 },
  unifiedControlsRow: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 12, marginBottom: 3 },
  unifiedSearchField: { flex: 1, minWidth: 0, height: 42, borderRadius: 15, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#FFFFFF", paddingHorizontal: 10, flexDirection: "row", alignItems: "center", gap: 6 },
  unifiedSearchInput: { flex: 1, minWidth: 0, color: "#38231C", fontSize: 11, paddingVertical: 0 },
  ...
  unifiedIconButtonActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  unifiedCartButton: { width: 42, height: 42, borderRadius: 15, backgroundColor: "#D76545", alignItems: "center", justifyContent: "center", position: "relative" },
  ...
  languageActive: { color: "#D76545" },
  languageSlash: { color: "#B9D6BB", fontSize: 11 },
  iconButton: { width: 39, height: 39, borderRadius: 14, backgroundColor: "#FFFFFF", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "#F1D8C8" },
  cartBadge: { position: "absolute", right: -3, top: -4, minWidth: 17, height: 17, borderRadius: 9, backgroundColor: "#D76545", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#FFF8F0" },
  cartBadgeText: { fontSize: 9, color: "#FFFFFF", fontWeight: "900" },
  announcementBoard: { backgroundColor: "#FFFFFF", minHeight: 190, borderRadius: 28, borderWidth: 1, borderColor: "#F4DCCB", padding: 18, overflow: "hidden", shadowColor: "#D76545", shadowOpacity: 0.08, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 2 },
  ...
  announcementHeaderIcon: { width: 34, height: 34, borderRadius: 12, backgroundColor: "#D76545", justifyContent: "center", alignItems: "center" },
  announcementSlideEyebrow: { color: "#D76545", fontSize: 10, fontWeight: "900", flexShrink: 1 },
  announcementSlideTitle: { color: "#38231C", fontSize: 22, lineHeight: 27, fontWeight: "900", maxWidth: 220 },
  announcementSlideBody: { color: "#7E665A", fontSize: 11, lineHeight: 17, marginTop: 7, maxWidth: 230 },
  announcementCta: { alignSelf: "flex-start", flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#D76545", borderRadius: 14, paddingHorizontal: 12, paddingVertical: 9, marginTop: 12 },
  ...
  announcementVisualCircle: { width: 88, height: 88, borderRadius: 44, backgroundColor: "#FFF1E5", borderWidth: 8, borderColor: "#F1D8C8", justifyContent: "center", alignItems: "center", transform: [{ rotate: "-8deg" }] },
  ...
  announcementDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#F1D8C8" },
  announcementDotActive: { width: 20, backgroundColor: "#D76545" },
  announcementNav: { flexDirection: "row", alignItems: "center", gap: 4 },
  announcementNavButton: { width: 27, height: 27, borderRadius: 10, backgroundColor: "#FFF4EB", justifyContent: "center", alignItems: "center" },
  announcementCounter: { color: "#7E665A", fontSize: 10, fontWeight: "800", minWidth: 25, textAlign: "center" },
  ...
  offerBadgeText: { color: "#38231C", fontSize: 9, fontWeight: "900" },
  ...
  offerKitchen: { color: "#FFF1E5", fontSize: 10 },
  ...
  heroCard: { backgroundColor: "#D76545", minHeight: 190, borderRadius: 28, padding: 20, flexDirection: "row", overflow: "hidden", position: "relative" },
  ...
  topSearchField: { flex: 1, height: 44, borderRadius: 16, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#FFFFFF", paddingHorizontal: 12, flexDirection: "row", alignItems: "center", gap: 7 },
  topSearchInput: { flex: 1, color: "#38231C", fontSize: 12, paddingVertical: 0 },
  topCartButton: { width: 44, height: 44, borderRadius: 16, backgroundColor: "#D76545", alignItems: "center", justifyContent: "center", position: "relative" },
  filterOnlyRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 5, marginBottom: 3 },
  filterHint: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  searchField: { flex: 1, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8", borderRadius: 16, height: 48, paddingHorizontal: 13, flexDirection: "row", alignItems: "center", gap: 8 },
  searchInput: { flex: 1, fontSize: 13, color: "#38231C", paddingVertical: 0 },
  ...
  filterButtonActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  filterPanel: { backgroundColor: "#FFFFFF", borderRadius: 18, borderWidth: 1, borderColor: "#F1D8C8", padding: 12, gap: 8 },
  filterTitle: { fontSize: 12, fontWeight: "900", color: "#38231C" },
  ...
  sortChipActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  sortChipText: { fontSize: 10, color: "#D76545", fontWeight: "900" },
  sortChipTextActive: { color: "#FFFFFF" },
  chipRow: { gap: 8 },
  chip: { borderRadius: 16, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: "#F3F9F1", borderWidth: 1, borderColor: "#F1D8C8" },
  ...
  sectionTitle: { fontSize: 17, fontWeight: "900", color: "#38231C" },
  sectionAction: { color: "#D76545", fontSize: 11, fontWeight: "900" },
  categoryRow: { gap: 9, paddingRight: 4 },
  categoryPill: { width: 75, paddingVertical: 11, paddingHorizontal: 8, borderRadius: 20, alignItems: "center", gap: 7, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8" },
  ...
  activeOrderId: { fontSize: 10, color: "#7E665A", fontWeight: "800" },
  activeOrderBody: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  activeOrderTitle: { color: "#38231C", fontSize: 14, fontWeight: "900" },
  activeOrderMeta: { color: "#7E665A", fontSize: 11, marginTop: 3 },
  kitchenRow: { gap: 12, paddingRight: 4 },
  kitchenCard: { width: 208, backgroundColor: "#FFFFFF", borderRadius: 22, borderWidth: 1, borderColor: "#F1D8C8", overflow: "hidden" },
  ...
  closedDot: { backgroundColor: "#CDB4A5" },
  ...
  kitchenName: { fontSize: 14, color: "#38231C", fontWeight: "900" },
  kitchenNeighborhood: { fontSize: 11, color: "#7E665A" },
  kitchenMeta: { flexDirection: "row", gap: 5, alignItems: "center", marginTop: 4 },
  kitchenSpecialty: { fontSize: 10, color: "#D76545", fontWeight: "900" },
  kitchenReviews: { color: "#CDB4A5", fontSize: 10 },
  mealList: { gap: 10 },
  mealRow: { backgroundColor: "#FFFFFF", borderRadius: 20, padding: 10, flexDirection: "row", alignItems: "center", gap: 10, borderWidth: 1, borderColor: "#F1D8C8" },
  ...
  mealPrep: { color: "#CDB4A5", fontSize: 10 },
  mealName: { fontSize: 14, color: "#38231C", fontWeight: "900" },
  mealDescription: { fontSize: 10, color: "#7E665A" },
  mealPrice: { color: "#D76545", fontSize: 12, fontWeight: "900", marginTop: 2 },
  addButton: { width: 31, height: 31, borderRadius: 12, backgroundColor: "#D76545", justifyContent: "center", alignItems: "center" },
  floatingCart: { position: "absolute", left: 18, right: 18, bottom: 24, borderRadius: 18, backgroundColor: "#38231C", paddingHorizontal: 16, paddingVertical: 13, flexDirection: "row", justifyContent: "space-between", alignItems: "center", shadowColor: "#38231C", shadowOpacity: 0.24, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 8 },
  floatingCartEyebrow: { color: "#CDB4A5", fontSize: 10, fontWeight: "700" },
  ...
  backButton: { width: 38, height: 38, borderRadius: 13, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8", justifyContent: "center", alignItems: "center" },
  pageTitle: { color: "#38231C", fontSize: 20, fontWeight: "900" },
  pageSubtitle: { color: "#7E665A", fontSize: 11, marginTop: 2 },
  clearButton: { marginLeft: "auto", paddingHorizontal: 7, paddingVertical: 7 },
  clearText: { color: "#D76545", fontSize: 11, fontWeight: "900" },
  ...
  profileStats: { backgroundColor: "#FFFFFF", borderRadius: 18, padding: 14, flexDirection: "row", justifyContent: "space-around", borderWidth: 1, borderColor: "#F1D8C8" },
  statItem: { alignItems: "center", gap: 3 },
  statValue: { color: "#38231C", fontSize: 15, fontWeight: "900" },
  statLabel: { color: "#7E665A", fontSize: 10 },
  storyCard: { flexDirection: "row", gap: 10, padding: 14, borderRadius: 18, backgroundColor: "#FFF1E5", borderWidth: 1, borderColor: "#F6D889" },
  storyIcon: { width: 38, height: 38, borderRadius: 14, backgroundColor: "#FFFFFF", justifyContent: "center", alignItems: "center" },
  storyCopy: { flex: 1 },
  storyTitle: { color: "#38231C", fontSize: 13, fontWeight: "900" },
  storyBody: { color: "#7E665A", fontSize: 11, lineHeight: 16, marginTop: 3 },
  cartItems: { gap: 10 },
  cartItemRow: { flexDirection: "row", gap: 11, padding: 10, backgroundColor: "#FFFFFF", borderRadius: 18, borderWidth: 1, borderColor: "#F1D8C8" },
  cartItemImage: { width: 75, height: 75, borderRadius: 15 },
  cartItemCopy: { flex: 1, justifyContent: "space-between", paddingVertical: 2 },
  cartItemName: { color: "#38231C", fontSize: 13, fontWeight: "900" },
  cartItemPrice: { color: "#D76545", fontSize: 12, fontWeight: "900" },
  ...
  quantityText: { fontSize: 12, color: "#38231C", fontWeight: "900" },
  ...
  complaintAddButton: { minHeight: 36, paddingHorizontal: 10, borderRadius: 13, backgroundColor: "#D76545", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 4 },
  ...
  complaintHeroTitle: { color: "#38231C", fontSize: 16, fontWeight: "900" },
  complaintHeroBody: { color: "#2E9B72", fontSize: 11, lineHeight: 17, marginTop: 3 },
  complaintFormCard: { padding: 15, backgroundColor: "#FFFFFF", borderRadius: 20, borderWidth: 1, borderColor: "#F1D8C8", gap: 10 },
  complaintFormTitle: { color: "#38231C", fontSize: 15, fontWeight: "900" },
  complaintCategoryGrid: { flexDirection: "row", flexWrap: "wrap", gap: 7 },
  complaintCategory: { minHeight: 38, paddingHorizontal: 9, borderRadius: 13, backgroundColor: "#F7FFF0", borderWidth: 1, borderColor: "#F1D8C8", flexDirection: "row", alignItems: "center", gap: 5 },
  complaintCategoryActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  complaintCategoryText: { color: "#1A4B52", fontSize: 10, fontWeight: "800" },
  complaintCategoryTextActive: { color: "#FFFFFF" },
  complaintSubjectInput: { minHeight: 46, borderRadius: 14, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#F7FFF0", color: "#38231C", fontSize: 11, paddingHorizontal: 12, paddingVertical: 9 },
  complaintDescriptionInput: { minHeight: 105, borderRadius: 14, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#F7FFF0", color: "#38231C", fontSize: 11, lineHeight: 17, paddingHorizontal: 12, paddingVertical: 10, textAlignVertical: "top" },
  complaintAttachLabel: { color: "#38231C", fontSize: 11, fontWeight: "900", marginTop: 2 },
  ...
  complaintAttachText: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  ...
  complaintsSectionHint: { color: "#7E665A", fontSize: 10, marginTop: 2 },
  complaintEmptyCard: { alignItems: "center", padding: 22, backgroundColor: "#F7FFF0", borderRadius: 20, borderWidth: 1, borderColor: "#F1D8C8", gap: 7 },
  complaintList: { gap: 10 },
  complaintCard: { padding: 14, backgroundColor: "#FFFFFF", borderRadius: 18, borderWidth: 1, borderColor: "#F1D8C8", gap: 9 },
  ...
  complaintCardTitle: { color: "#38231C", fontSize: 13, fontWeight: "900", marginTop: 2 },
  ...
  complaintStatusText: { color: "#7E665A", fontSize: 9, fontWeight: "900" },
  ...
  complaintResponseText: { flex: 1, color: "#D76545", fontSize: 10, lineHeight: 15, fontWeight: "800" },
  disabledButton: { opacity: 0.55 },
  complaintInbox: { backgroundColor: "#FFFFFF", borderRadius: 18, borderWidth: 1, borderColor: "#F1D8C8", overflow: "hidden" },
  ...
  complaintInboxTitle: { color: "#38231C", fontSize: 11, fontWeight: "900" },
  complaintInboxMeta: { color: "#7E665A", fontSize: 9, marginTop: 2 },
  complaintInboxAction: { paddingHorizontal: 9, paddingVertical: 7, borderRadius: 10, backgroundColor: "#EEF9DB" },
  complaintInboxActionText: { color: "#D76545", fontSize: 9, fontWeight: "900" },
  supportEmptyCard: { flexDirection: "row", alignItems: "center", gap: 8, padding: 13, backgroundColor: "#F7FFF0", borderRadius: 16, borderWidth: 1, borderColor: "#F1D8C8" },
  supportEmptyText: { color: "#2E9B72", fontSize: 10, fontWeight: "800" },
  cartNoteCard: { padding: 14, backgroundColor: "#F7FFF0", borderRadius: 18, borderWidth: 1, borderColor: "#F1D8C8", gap: 10 },
  cartNoteHeader: { flexDirection: "row", alignItems: "flex-start", gap: 8 },
  cartNoteCopy: { flex: 1 },
  cartNoteTitle: { color: "#38231C", fontSize: 13, fontWeight: "900" },
  cartNoteHint: { color: "#7E665A", fontSize: 11, lineHeight: 16, marginTop: 2 },
  deliveryIcon: { width: 38, height: 38, borderRadius: 14, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center" },
  deliveryCopy: { flex: 1 },
  deliveryTitle: { fontSize: 13, color: "#38231C", fontWeight: "900" },
  deliveryBody: { fontSize: 11, color: "#2E9B72", marginTop: 2 },
  summaryCard: { padding: 15, backgroundColor: "#FFFFFF", borderRadius: 18, borderWidth: 1, borderColor: "#F1D8C8", gap: 10 },
  summaryRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  summaryLabel: { fontSize: 12, color: "#7E665A" },
  summaryValue: { fontSize: 12, color: "#1A4B52", fontWeight: "800" },
  summaryStrong: { color: "#38231C", fontSize: 15, fontWeight: "900" },
  summaryDivider: { height: 1, backgroundColor: "#E8F1E6" },
  primaryButton: { minHeight: 52, borderRadius: 17, backgroundColor: "#D76545", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  ...
  secondaryButtonText: { color: "#D76545", fontSize: 12, fontWeight: "900" },
  modalBackdrop: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(28,25,23,0.38)" },
  checkoutSheet: { backgroundColor: "#FFF8F0", borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 18, paddingBottom: 28, gap: 13 },
  customizationSheet: { backgroundColor: "#FFF8F0", borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 18, paddingBottom: 28, gap: 12, maxHeight: "92%" },
  customizationScroll: { maxHeight: 520 },
  customizationContent: { gap: 10, paddingBottom: 2 },
  customizationMealHeader: { flexDirection: "row", alignItems: "center", gap: 11, padding: 10, backgroundColor: "#FFFFFF", borderRadius: 17, borderWidth: 1, borderColor: "#F1D8C8" },
  customizationMealImage: { width: 62, height: 62, borderRadius: 15 },
  customizationMealCopy: { flex: 1, gap: 3 },
  customizationMealName: { color: "#38231C", fontSize: 14, fontWeight: "900" },
  customizationMealPrice: { color: "#D76545", fontSize: 12, fontWeight: "900" },
  customizationHint: { color: "#7E665A", fontSize: 10, lineHeight: 14 },
  sheetHandle: { width: 38, height: 4, borderRadius: 3, backgroundColor: "#D6E2D4", alignSelf: "center" },
  sheetHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  sheetEyebrow: { fontSize: 10, color: "#D76545", fontWeight: "900", letterSpacing: 1 },
  sheetTitle: { fontSize: 22, fontWeight: "900", color: "#38231C", marginTop: 2 },
  closeButton: { width: 35, height: 35, borderRadius: 12, backgroundColor: "#FFFFFF", justifyContent: "center", alignItems: "center" },
  optionLabel: { color: "#38231C", fontSize: 12, fontWeight: "900", marginTop: 4 },
  optionRow: { flexDirection: "row", gap: 8 },
  optionCard: { flex: 1, borderRadius: 16, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#FFFFFF", padding: 11, gap: 5 },
  optionCardActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  optionCardTitle: { fontSize: 11, color: "#38231C", fontWeight: "900" },
  optionCardTitleActive: { color: "#FFFFFF" },
  optionCardSubtitle: { fontSize: 9, color: "#7E665A" },
  optionCardSubtitleActive: { color: "#E6F9C7" },
  paymentList: { gap: 7 },
  ingredientGroupLabel: { color: "#7E665A", fontSize: 10, fontWeight: "900", marginTop: 2 },
  ingredientOptionGrid: { flexDirection: "row", flexWrap: "wrap", gap: 7 },
  ingredientOption: { width: "48%", minHeight: 42, flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 9, paddingVertical: 8, borderRadius: 13, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8" },
  ingredientOptionSelected: { backgroundColor: "#D76545", borderColor: "#D76545" },
  ...
  specialRequestInputWrap: { minHeight: 72, flexDirection: "row", alignItems: "flex-start", gap: 8, backgroundColor: "#FFFFFF", borderRadius: 16, borderWidth: 1, borderColor: "#F1D8C8", padding: 11 },
  specialRequestInput: { flex: 1, minHeight: 64, color: "#38231C", fontSize: 11, lineHeight: 17, padding: 0, textAlignVertical: "top" },
  paymentOption: { flexDirection: "row", alignItems: "center", gap: 10, padding: 10, borderRadius: 16, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8" },
  ...
  paymentIconActive: { backgroundColor: "#D76545" },
  paymentCopy: { flex: 1 },
  paymentTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  paymentSubtitle: { color: "#7E665A", fontSize: 10, marginTop: 2 },
  multiOrderSummary: { backgroundColor: "#FFF1E5", borderRadius: 16, padding: 12, gap: 7, borderWidth: 1, borderColor: "#F1D8C8" },
  multiOrderSummaryTitle: { color: "#38231C", fontSize: 11, fontWeight: "900" },
  multiOrderRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingTop: 5, borderTopWidth: 1, borderTopColor: "#F1D8C8" },
  multiOrderKitchen: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  multiOrderTotal: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  ...
  sheetTotalLabel: { color: "#7E665A", fontSize: 12 },
  sheetTotalValue: { color: "#38231C", fontSize: 18, fontWeight: "900" },
  activeOrdersPanel: { backgroundColor: "#FFFFFF", borderRadius: 18, padding: 12, borderWidth: 1, borderColor: "#F1D8C8", gap: 8 },
  activeOrdersTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  activeOrdersRow: { gap: 8, paddingRight: 2 },
  activeOrderChip: { minWidth: 118, maxWidth: 155, backgroundColor: "#FFF8F0", borderRadius: 13, paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: "#F7E7DB", gap: 3 },
  activeOrderChipActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  activeOrderChipId: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  activeOrderChipKitchen: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  activeOrderChipTextActive: { color: "#FFFFFF" },
  orderHero: { padding: 16, backgroundColor: "#38231C", borderRadius: 20, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  orderHistoryCard: { backgroundColor: "#FFFFFF", borderRadius: 20, padding: 15, borderWidth: 1, borderColor: "#F1D8C8", gap: 10 },
  orderHistoryHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 },
  orderHistoryHint: { color: "#7E665A", fontSize: 10, marginTop: 3 },
  ...
  orderHistoryId: { color: "#D76545", fontSize: 11, fontWeight: "900" },
  orderHistoryKitchen: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  orderHistoryItems: { color: "#7E665A", fontSize: 10, lineHeight: 15 },
  orderHistoryMeta: { color: "#8A6516", fontSize: 10, fontWeight: "800" },
  reorderButton: { minHeight: 38, borderRadius: 13, backgroundColor: "#D76545", paddingHorizontal: 10, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5 },
  ...
  scheduleClosedTitle: { color: "#A6523E", fontSize: 11, fontWeight: "900" },
  scheduleClosedBody: { color: "#9A6B5A", fontSize: 10 },
  scheduleCard: { backgroundColor: "#FFFFFF", borderRadius: 20, padding: 15, borderWidth: 1, borderColor: "#F1D8C8", gap: 12 },
  scheduleHeader: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 8 },
  scheduleHeaderCopy: { flex: 1 },
  scheduleTitle: { color: "#38231C", fontSize: 14, fontWeight: "900" },
  scheduleHint: { color: "#7E665A", fontSize: 10, marginTop: 3 },
  ...
  scheduleDownloadText: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  ...
  scheduleToggleTitle: { color: "#38231C", fontSize: 11, fontWeight: "900" },
  scheduleToggleHint: { color: "#7E665A", fontSize: 10, marginTop: 3 },
  scheduleEditor: { gap: 9, paddingTop: 2 },
  scheduleGroupLabel: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  weekdayGrid: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
  weekdayChip: { minHeight: 34, borderRadius: 11, backgroundColor: "#F0FBEA", paddingHorizontal: 8, flexDirection: "row", alignItems: "center", gap: 4, borderWidth: 1, borderColor: "#F1D8C8" },
  weekdayChipClosed: { backgroundColor: "#FFF1EB", borderColor: "#F0C7B7" },
  weekdayChipText: { color: "#D76545", fontSize: 9, fontWeight: "900" },
  weekdayChipTextClosed: { color: "#A6523E" },
  mealScheduleRow: { gap: 7, paddingVertical: 8, borderTopWidth: 1, borderTopColor: "#EFF6ED" },
  mealScheduleCopy: { gap: 2 },
  mealScheduleName: { color: "#38231C", fontSize: 11, fontWeight: "900" },
  mealScheduleMeta: { color: "#7E665A", fontSize: 9 },
  mealDayMiniGrid: { flexDirection: "row", gap: 5 },
  mealDayMini: { width: 27, height: 27, borderRadius: 9, backgroundColor: "#F7FFF0", borderWidth: 1, borderColor: "#F1D8C8", alignItems: "center", justifyContent: "center" },
  mealDayMiniActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  mealDayMiniDisabled: { opacity: 0.4 },
  mealDayMiniText: { color: "#7E665A", fontSize: 9, fontWeight: "900" },
  mealDayMiniTextActive: { color: "#F6D889" },
  orderHeroEyebrow: { color: "#CDB4A5", fontSize: 10, fontWeight: "800" },
  orderHeroId: { color: "#FFFFFF", fontSize: 21, fontWeight: "900", marginTop: 3 },
  orderEta: { alignItems: "flex-end" },
  orderEtaLabel: { color: "#CDB4A5", fontSize: 10 },
  ...
  trackingCard: { backgroundColor: "#FFFFFF", borderRadius: 20, padding: 16, borderWidth: 1, borderColor: "#F1D8C8" },
  trackingTitle: { color: "#38231C", fontSize: 16, fontWeight: "900", marginBottom: 15 },
  ...
  trackCopy: { flex: 1 },
  trackLabel: { color: "#7E665A", fontSize: 12, fontWeight: "800" },
  trackLabelActive: { color: "#38231C", fontWeight: "900" },
  trackCaption: { color: "#CDB4A5", fontSize: 10, marginTop: 2 },
  ...
  specialRequestBody: { color: "#7E665A", fontSize: 11, lineHeight: 17, marginTop: 3 },
  deliveredCard: { flexDirection: "row", gap: 9, alignItems: "center", backgroundColor: "#FFF1E5", borderRadius: 16, padding: 13 },
  ...
  ratingCopy: { flex: 1 },
  ratingTitle: { color: "#38231C", fontSize: 13, fontWeight: "900" },
  ratingBody: { color: "#7E665A", fontSize: 10, marginTop: 3 },
  ratingStarsRow: { flexDirection: "row", justifyContent: "center", gap: 4 },
  ratingStarButton: { padding: 2 },
  ratingInput: { minHeight: 58, borderRadius: 14, borderWidth: 1, borderColor: "#F1D8C8", backgroundColor: "#F7FFF0", color: "#38231C", fontSize: 11, lineHeight: 17, padding: 10 },
  ratingSubmit: { minHeight: 45, borderRadius: 14, backgroundColor: "#D76545", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 },
  ratingSubmitDisabled: { backgroundColor: "#CDB4A5" },
  ...
  driverOrdersQueue: { backgroundColor: "#FFFFFF", borderRadius: 18, padding: 12, borderWidth: 1, borderColor: "#F1D8C8", gap: 8 },
  driverOrdersQueueTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  driverOrdersQueueRow: { gap: 8, paddingRight: 2 },
  driverOrderChip: { minWidth: 118, maxWidth: 155, backgroundColor: "#FFF8F0", borderRadius: 13, paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: "#F7E7DB", gap: 3 },
  driverOrderChipActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  driverOrderChipId: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  driverOrderChipKitchen: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  driverOrderChipTextActive: { color: "#FFFFFF" },
  incomingQueue: { backgroundColor: "#FFFFFF", borderRadius: 18, padding: 12, borderWidth: 1, borderColor: "#F1D8C8", gap: 8 },
  incomingQueueTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  incomingQueueRow: { gap: 8, paddingRight: 2 },
  incomingQueueChip: { minWidth: 118, maxWidth: 155, backgroundColor: "#FFF8F0", borderRadius: 13, paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: "#F7E7DB", gap: 3 },
  incomingQueueChipActive: { backgroundColor: "#D76545", borderColor: "#D76545" },
  incomingQueueChipId: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  incomingQueueChipKitchen: { color: "#7E665A", fontSize: 10, fontWeight: "800" },
  incomingQueueChipTextActive: { color: "#FFFFFF" },
  earningsRow: { flexDirection: "row", gap: 8 },
  dashboardMetric: { flex: 1, borderRadius: 16, padding: 10, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F1D8C8", gap: 4 },
  dashboardMetricValue: { color: "#38231C", fontSize: 15, fontWeight: "900" },
  dashboardMetricLabel: { color: "#7E665A", fontSize: 9 },
  ...
  incomingEyebrow: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  incomingId: { color: "#38231C", fontSize: 17, fontWeight: "900", marginTop: 2 },
  newPill: { backgroundColor: "#F0FBEA", borderRadius: 10, paddingHorizontal: 8, paddingVertical: 5 },
  newPillText: { color: "#D76545", fontSize: 9, fontWeight: "900" },
  incomingTitle: { color: "#38231C", fontSize: 14, fontWeight: "900" },
  incomingMeta: { color: "#7E665A", fontSize: 11 },
  ...
  rejectText: { color: "#7E665A", fontSize: 12, fontWeight: "900" },
  acceptButton: { flex: 1, height: 44, borderRadius: 14, backgroundColor: "#D76545", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6 },
  ...
  dashboardList: { backgroundColor: "#FFFFFF", borderRadius: 20, borderWidth: 1, borderColor: "#F1D8C8", overflow: "hidden" },
  ...
  dashboardActionTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  dashboardActionDetail: { color: "#7E665A", fontSize: 10, marginTop: 3 },
  ...
  menuManagerName: { color: "#38231C", fontSize: 11, fontWeight: "900" },
  menuManagerMeta: { color: "#7E665A", fontSize: 10, marginTop: 2 },
  ...
  cliqCopy: { flex: 1 },
  cliqTitle: { color: "#38231C", fontSize: 12, fontWeight: "900" },
  ...
  profileGreeting: { color: "#38231C", fontSize: 17, fontWeight: "900" },
  profileMuted: { color: "#7E665A", fontSize: 11, marginTop: 3 },
  ...
  switchRoleText: { color: "#D76545", fontSize: 10, fontWeight: "900" },
  ...
  settingsCard: { backgroundColor: "#FFFFFF", borderRadius: 20, borderWidth: 1, borderColor: "#F1D8C8", overflow: "hidden" },
  ...
  settingLabel: { color: "#38231C", fontSize: 12, fontWeight: "800", flex: 1 },
  settingValue: { color: "#7E665A", fontSize: 11 },
  aboutCard: { borderRadius: 20, padding: 16, backgroundColor: "#38231C" },
  ...
  emptyTitle: { color: "#38231C", fontSize: 17, fontWeight: "900" },
  emptyBody: { color: "#7E665A", fontSize: 11, textAlign: "center", maxWidth: 220, lineHeight: 17 },
  bottomNav: { position: "absolute", left: 14, right: 14, bottom: 13, height: 69, backgroundColor: "rgba(255,255,255,0.97)", borderRadius: 23, borderWidth: 1, borderColor: "#F1D8C8", flexDirection: "row", alignItems: "center", justifyContent: "space-around", shadowColor: "#38231C", shadowOpacity: 0.08, shadowRadius: 14, shadowOffset: { width: 0, height: 6 }, elevation: 7 },
  navItem: { alignItems: "center", justifyContent: "center", gap: 4, minWidth: 55, paddingVertical: 7 },
  navLabel: { color: "#CDB4A5", fontSize: 9, fontWeight: "800" },
  navLabelActive: { color: "#D76545" },
  navBrandDot: { width: 42, height: 42, borderRadius: 16, backgroundColor: "#D76545", alignItems: "center", justifyContent: "center", marginTop: -26, borderWidth: 4, borderColor: "#FFF8F0" },
  toast: { position: "absolute", left: 24, right: 24, bottom: 98, borderRadius: 15, backgroundColor: "#38231C", paddingHorizontal: 13, paddingVertical: 12, flexDirection: "row", alignItems: "center", gap: 8, shadowColor: "#38231C", shadowOpacity: 0.25, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 8 },
  toastText: { color: "#FFFFFF", fontSize: 11, fontWeight: "800", flex: 1 },
  customerDriverCard: { backgroundColor: "#FFF1E5", borderRadius: 22, padding: 15, gap: 14, borderWidth: 1, borderColor: "#F6D889" },
  customerDriverHeader: { flexDirection: "row", alignItems: "center", gap: 11 },
  driverAvatar: { width: 44, height: 44, borderRadius: 16, backgroundColor: "#D76545", alignItems: "center", justifyContent: "center" },
  ...
  customerDriverName: { color: "#38231C", fontSize: 15, fontWeight: "900" },
  customerDriverMeta: { color: "#7E665A", fontSize: 11, fontWeight: "600" },
  ...
  customerDriverStatLabel: { color: "#CDB4A5", fontSize: 9, fontWeight: "800", marginBottom: 3 },
