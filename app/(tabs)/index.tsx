...
function MotherDashboard({ onBack }: { onBack: () => void }) {
  ...
  const saveKitchenDescriptionMutation = trpc.kitchens.updateDescription.useMutation({
    onSuccess: (saved) => {
      saveLocalKitchenDescription(saved.kitchenId, { ar: saved.descriptionAr, en: saved.descriptionEn });
  ...
  const fallbackDescription = kitchenDescriptions["umm-ahmad"] ?? kitchens.find((kitchen) => kitchen.id === "umm-ahmad")?.description ?? { ar: "أكلات بيتية طازجة نحضرها يومياً بحب.", en: "Fresh home-cooked dishes prepared daily with care." };
  const remoteDescription = kitchenDescriptionQuery.data;
  const hasRemoteDescription = Boolean(remoteDescription && (remoteDescription.descriptionAr.trim() || remoteDescription.descriptionEn.trim()));
  ...
  }, [currentDescription.ar, currentDescription.en]);
  const saveDescription = () => {
    const nextAr = descriptionAr.trim();
    const nextEn = descriptionEn.trim();
    if (!nextAr && !nextEn) {
      showToast(language === "ar" ? "اكتبي وصفاً واحداً على الأقل" : "Add at least one description");
      return;
    }
    saveLocalKitchenDescription("umm-ahmad", { ar: nextAr, en: nextEn });
    if (user) {
      saveKitchenDescriptionMutation.mutate({ kitchenId: "umm-ahmad", descriptionAr: nextAr, descriptionEn: nextEn });
      return;
    }
    setDescriptionEditorOpen(false);
