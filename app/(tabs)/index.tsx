    ...
    <ScreenContainer edges={["top", "left", "right", "bottom"]} containerClassName="bg-background" className="flex-1">
      <ScrollView contentContainerStyle={styles.loginScroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.loginCard}>
          ...
          {!isReset && <><Text style={styles.inputLabel}>{language === "ar" ? "كلمة المرور" : "Password"}</Text><View style={styles.inputWrap}><MaterialIcons name="lock-outline" size={18} color="#00AFC4" /><TextInput value={password} onChangeText={setPassword} editable={otpStep === "credentials"} placeholder={language === "ar" ? "٨ أحرف على الأقل" : "At least 8 characters"} placeholderTextColor="#8ABAC0" secureTextEntry style={styles.loginInput} textAlign={language === "ar" ? "right" : "left"} /></View>{!isCreate && otpStep === "credentials" && <Pressable onPress={() => { setIsReset(true); resetFlow(); }} style={({ pressed }) => [styles.passwordRecoveryLink, pressed && styles.pressed]}><MaterialIcons name="lock-reset" size={16} color="#00AFC4" /><Text style={styles.passwordRecoveryText}>{language === "ar" ? "نسيت كلمة المرور؟ استرجعيها برمز الهاتف" : "Forgot your password? Reset it by phone"}</Text></Pressable>}</>}
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      {cart.length === 0 ? <EmptyCart language={language} onBack={onBack} /> : <>
        ...
        <Pressable onPress={() => {
          // Open quick location selector modal or jump to checkout map picker
          onCheckout();
        }} style={({ pressed }) => [styles.deliveryCard, pressed && styles.pressed]}>
          <View style={styles.deliveryIcon}><MaterialIcons name="map" size={21} color="#00AFC4" /></View>
          <View style={styles.deliveryCopy}>
            <Text style={styles.deliveryTitle}>{language === "ar" ? "توصيل لباب البيت (اضغطي لتحديد الموقع على الخريطة)" : "Doorstep delivery (Tap to pick location on map)"}</Text>
            <Text style={styles.deliveryBody}>{language === "ar" ? "انقري لاختيار موقع التوصيل المختلف بدقة" : "Tap to select exact delivery spot"}</Text>
          </View>
          <MaterialIcons name="chevron-right" size={20} color="#00AFC4" />
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  passwordRecoveryLink: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 7, paddingHorizontal: 8, borderRadius: 12, backgroundColor: "#F0FBFC", borderWidth: 1, borderColor: "#C6EDEF" },
  passwordRecoveryText: { color: "#00AFC4", fontSize: 11, fontWeight: "900", textAlign: "center" },
