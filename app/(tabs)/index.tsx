...
function MealsScreen({ onBack, onOpenCart, onOpenKitchen }: { onBack: () => void; onOpenCart: () => void; onOpenKitchen: (kitchenId: string) => void }) {
  const { language, selectedRegion, selectedCategory, setSelectedCategory, addToCart, cart, cartCount, cartTotal } = useApp();
  const region = getRegion(selectedRegion);
    ...
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.mealList}>
        {visibleMeals.map((meal) => (
          <MealRow key={meal.id} meal={meal} language={language} quantity={cart.find((item) => item.meal.id === meal.id)?.quantity ?? 0} onPress={() => openKitchen(meal.kitchenId)} onAdd={() => addToCart(meal)} />
        ))}
      </View>
