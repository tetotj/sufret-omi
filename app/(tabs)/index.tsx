...
type AnnouncementSlide = { id: string; icon: IconName; eyebrowAr: string; eyebrowEn: string; titleAr: string; titleEn: string; bodyAr: string; bodyEn: string; ctaAr: string; ctaEn: string; target: "meals" | "orders" };
...
function CustomerHome({
  ...
  const announcementsQuery = trpc.marketing.announcements.useQuery();
  const offersQuery = trpc.marketing.offers.useQuery();
  const announcements = useMemo<AnnouncementSlide[]>(() => announcementsQuery.data !== undefined ? announcementsQuery.data.map((item) => ({ ...item, icon: item.icon as IconName })) : FALLBACK_ANNOUNCEMENTS, [announcementsQuery.data]);
    ...
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.mealList}>
        {visibleMeals.map((meal) => (
          <MealRow key={meal.id} meal={meal} language={language} offerBadge={offersOnly ? offerBadges.get(meal.id) : undefined} quantity={cart.find((item) => item.meal.id === meal.id)?.quantity ?? 0} onRemove={() => updateQuantity(meal.id, (cart.find((item) => item.meal.id === meal.id)?.quantity ?? 1) - 1)} onPress={() => openKitchen(meal.kitchenId)} onAdd={() => onRequestAdd(meal)} />
        ))}
      </View>
